...
exports.analyzeComposition = analyzeComposition;
...
function analyzeComposition(input) {
    const principles = (0, knowledge_base_1.searchPrinciples)({ industry: input.industry, category: 'composition' });
    ...
    const scored = LAYOUTS.map((layout) => {
        ...
        if (input.markType === 'wordmark' && layout.id === 'comp-horizontal-lockup')
            score += 2;
        if (input.markType === 'symbol' && layout.id === 'comp-centered')
            score += 2;
        if (input.hasNegativeSpace && layout.id === 'comp-negative-space')
            score += 3;
        if (input.elementCount && input.elementCount > 2 && layout.id === 'comp-overlay')
            score += 2;
...
function buildHierarchy(markType) {
    const base = ['Primary mark draws eye first', 'Secondary text supports recognition'];
    if (markType === 'combination')
        return [...base, 'Symbol and wordmark equal weight at large scale', 'Symbol dominates at small scale'];
    if (markType === 'wordmark')
        return ['Typography is the sole recognition element', 'Letterform distinctiveness is critical'];
    if (markType === 'emblem')
        return [...base, 'Contained within boundary shape', 'Works as stamp or seal'];
    return [...base, 'Symbol must work standalone without text'];
}
//# sourceMappingURL=composition-ai.engine.js.map