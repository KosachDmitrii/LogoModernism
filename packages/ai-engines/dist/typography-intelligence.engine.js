...
function analyzeTypography(input) {
    const principles = (0, knowledge_base_1.searchPrinciples)({ industry: input.industry, category: 'typography' });
    const principleIds = new Set(principles.map((p) => p.id));
    const scored = TYPOGRAPHY_PROFILES.map((profile) => {
        let score = profile.score;
        if (principleIds.has(profile.principleId))
            score += 2;
        if (input.style === 'swiss' && profile.name.includes('Swiss'))
            score += 3;
        if (input.style === 'bauhaus' && profile.category === 'custom')
            score += 2;
        if (input.markType === 'wordmark' && profile.category === 'wordmark')
            score += 4;
        if (input.markType === 'lettermark' && profile.category === 'monogram')
            score += 4;
        if (input.companyName.length <= 4 && profile.category === 'monogram')
            score += 2;
...
//# sourceMappingURL=typography-intelligence.engine.js.map