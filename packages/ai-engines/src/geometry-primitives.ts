...
export const GEOMETRY_PRIMITIVES: GeometryPrimitive[] = [
  {
    id: 'prim-circle',
    name: 'Circle',
    category: 'circle',
...
export function getPrimitiveById(id: string): GeometryPrimitive | undefined {
  return GEOMETRY_PRIMITIVES.find((p) => p.id === id);
}

export function getPrimitivesByCategory(category: GeometryPrimitive['category']): GeometryPrimitive[] {
  return GEOMETRY_PRIMITIVES.filter((p) => p.category === category);
}
