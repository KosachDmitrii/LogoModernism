...
const SHAPE_PSYCHOLOGY: Record<string, { emotions: string[]; associations: string[]; industries: string[] }> = {
  circle: {
...
export function analyzeShapePsychology(input: ShapePsychologyInput): ShapePsychologyResult[] {
...
function getCulturalNotes(shape: string): string[] {
...
function buildRecommendation(shape: string, personality?: string): string {
  if (personality === 'playful' && shape === 'organic') return 'Use soft organic forms with asymmetric balance';
  if (personality === 'technical' && shape === 'hexagon') return 'Combine hexagonal grid with precise monoline construction';
  if (personality === 'luxurious' && shape === 'diamond') return 'Pair diamond geometry with generous negative space';
  return `Leverage ${shape} psychology with modernist construction discipline`;
}

export function getShapePsychologyForPrimitive(primitiveId: string): ShapePsychologyResult | null {
  const prim = GEOMETRY_PRIMITIVES.find((p) => p.id === primitiveId);
  if (!prim) return null;
