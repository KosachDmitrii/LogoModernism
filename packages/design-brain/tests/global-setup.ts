...
import { pushSchema, ensureBrainSchema, createTestDatabase } from './helpers/db';
...
export async function setup(): Promise<() => Promise<void>> {
  ...
  if (!databaseUrl && process.env.BRAIN_USE_TESTCONTAINERS !== '0') {
    try {
      ...
      console.info(`[brain-tests] Started pgvector container at ${host}:${port}`);
    } catch (error) {
      console.warn(
        '[brain-tests] Could not start testcontainers — integration tests will be skipped.',
  ...
  try {
    ...
    console.info('[brain-tests] Database ready for integration tests');
  } catch (error) {
    process.env.BRAIN_DB_READY = 'false';
    console.warn(
      '[brain-tests] Database setup failed — integration tests will be skipped.',
      error instanceof Error ? error.message : error,
    );
  }
