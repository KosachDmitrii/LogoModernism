...
exports.initialsFromName = void 0;
exports.enhanceLogoPrompt = enhanceLogoPrompt;
exports.resolveMarkTypeFromPrompt = resolveMarkTypeFromPrompt;
const shared_1 = require("@logo-platform/shared");
Object.defineProperty(exports, "initialsFromName", { enumerable: true, get: function () { return shared_1.lettermarkTextFromName; } });
const ICON_SUFFIX = ' Professional logo design, flat vector style, clean white background, ' +
    'scalable icon, modernist Swiss design, ' +
    'no gradients, no shadows, no photorealism, centered composition.';
const CONSTRUCTED_SUFFIX = ' Professional constructed typography logo, letters built from geometric primitives — ' +
...
const WORDMARK_SUFFIX = ' Professional typographic wordmark logo, letters spelling the brand name are the entire logo, ' +
...
function lettermarkSuffix(companyName) {
    const text = (0, shared_1.lettermarkTextFromName)(companyName);
    const usesInitials = (0, shared_1.isMultiWordCompanyName)(companyName);
    if (usesInitials) {
        return (` Professional lettermark monogram logo built only from the initials "${text}", ` +
    ...
    return (` Professional lettermark logo built from the full word "${text}", ` +
        'the full word is the entire logo, do not abbreviate to initials, ' +
        'no icon, no pictorial symbol, no emblem, no badge, no industry imagery, ' +
        'flat vector letterforms, clean white background, black on white, ' +
        'no gradients, no shadows, no photorealism, centered composition.');
}
function detectMarkType(request) {
    if (request.markType)
        return request.markType;
    ...
    if (text.includes('wordmark only') ||
...
function enhanceLogoPrompt(request) {
    ...
    const isConstructed = request.typographyStyle === 'constructed' ||
...
function resolveMarkTypeFromPrompt(text) {
    const lower = text.toLowerCase();
    if (lower.includes('wordmark only') || lower.includes('typography-only wordmark')) {
        return 'wordmark';
    }
    if (lower.includes('lettermark'))
        return 'lettermark';
    if (lower.includes('combination mark'))
        return 'combination';
    return undefined;
}
