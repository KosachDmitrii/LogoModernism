...
function resolveIndustryRules(industry) {
    ...
    const keywordRules = knowledge_base_1.designPrinciples.filter((p) => p.tags.some((t) => normalized.includes(t) || t.includes(normalized)) ||
...
function selectDesignRules(input) {
    ...
    if (input.preferredEra) {
        ...
        addRule((0, knowledge_base_1.getPrincipleById)(eraId));
    }
    else {
        const eraRules = knowledge_base_1.designPrinciples.filter((p) => p.category === 'era');
        addRule(pickWeighted(eraRules, rand, 1)[0]);
    }
    for (const category of CATEGORY_ORDER) {
        if (['industry', 'era', 'inspiration'].includes(category))
            continue;
        const pool = knowledge_base_1.designPrinciples.filter((p) => {
...
function buildLogoDNA(principles, input) {
